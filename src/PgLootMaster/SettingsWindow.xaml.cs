...
public partial class SettingsWindow : Window
{
    ...
    private static readonly string[] StrategyDescriptions = new[]
    {
        ...
        "Aggressively prioritizes matches of the item you select in the toolbar (20× multiplier vs 5× baseline). Uses the real per-match scoring formula by variant so the 20× scales actual match values, not the old ad-hoc constants. Will sacrifice raw score to capture your target. Depends on item recognition — double-check the toolbar dropdown shows the right item before relying on this.",
        "Real reverse-engineered per-match scoring by variant (Loot Master 2N−3, Deluxe 3N−6; +capture bonus), layered on Cascade Hunter's 2-ply philosophy. Adds a tier-unlock term: a move that pushes the running capture count into a new bonus tier is valued for its permanent uplift on every future match.",
        "Cascade Hunter's levers pushed harder (base 0.95, decay 0.95, bottom-row 3.0, beam 8) — bet on per-move density (~45 pts/move is the leaderboard ceiling). Plus a per-variant Tier Hold: once captures hit the last bonus-tier change (Deluxe 3, Loot Master 2), suppress matches that would walk into the next capture (no bonus uplift, only board dilution).",
    };
